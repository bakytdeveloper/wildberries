...
//                     setSuggestions(uniqueQueries.map(item => ({ label: item.toString() }))); // Преобразуем строки в объекты
//
//                     const uniqueArticles = [...new Set(savedQueries.flatMap(query => query.article.split('; ').filter(Boolean)))];
//                     setArticleSuggestions(uniqueArticles.map(item => ({ label: item.toString() }))); // Преобразуем строки в объекты
//                 }
//             } catch (jsonError) {
...
//         console.log('Query selected:', selected); // Логируем выбранные данные
//         const value = selected.length > 0 ? selected[0].label : ''; // Извлекаем значение label
//         setRequestForms(requestForms.map(f => f.id === formId ? { ...f, query: value } : f));
//     };
//
//     const handleArticleChange = (selected, formId) => {
//         console.log('Article selected:', selected); // Логируем выбранные данные
//         const value = selected.length > 0 ? selected[0].label : ''; // Извлекаем значение label
//         setRequestForms(requestForms.map(f => f.id === formId ? { ...f, article: value } : f));
//     };
//
//     const handleQueryInputChange = (text, formId) => {
//         console.log('Query input change:', text); // Логируем ввод вручную
//         setRequestForms(requestForms.map(f => f.id === formId ? { ...f, query: text } : f));
//     };
//
//     const handleArticleInputChange = (text, formId) => {
//         console.log('Article input change:', text); // Логируем ввод вручную
...
//         if (isRequesting) return;
//
//         // Логируем текущие значения форм
//         console.log('Request forms before validation:', requestForms);
//
//         const validForms = requestForms.filter(form => {
//             const query = form.query ? (typeof form.query === 'string' ? form.query.trim() : form.query.label || form.query).trim() : '';
//             const article = form.article ? (typeof form.article === 'string' ? form.article.trim() : form.article.label || form.article).trim() : '';
//             return query !== '' && article !== '';
//         });
//
//         console.log('Valid forms after validation:', validForms); // Логируем валидные формы
//
//         if (validForms.length === 0) {
//             Toastify({ text: "Все формы должны быть заполнены.", duration: 3000, gravity: "top", position: "right", style: { background: '#ff0000' } }).showToast();
//             return;
...
//                 ...form,
//                 query: form.query ? (typeof form.query === 'string' ? form.query.trim() : form.query.label || form.query).trim() : '',
//                 article: form.article ? (typeof form.article === 'string' ? form.article.trim() : form.article.label || form.article).trim() : '',
//                 dest: cityDestinations[form.city],
//                 city: form.city,
//                 queryTime: new Date().toISOString()
//             }));
//
//             console.log('Trimmed forms before sending:', trimmedForms); // Логируем данные перед отправкой
//
//             const response = await fetch(`${API_HOST}/api/article`, {
//                 method: 'POST',
//                 headers: {
...
//             setFilteredQueries([result, ...allQueries]);
//
//             const newQueries = validForms.map(form => form.query ? (typeof form.query === 'string' ? form.query.trim() : form.query.label || form.query).trim() : '');
//             const newSuggestions = [...suggestions];
//             newQueries.forEach(query => {
//                 if (!newSuggestions.includes(query)) {
//                     newSuggestions.push(query);
...
//             setSuggestions(newSuggestions);
//
//             const newArticles = validForms.map(form => form.article ? (typeof form.article === 'string' ? form.article.trim() : form.article.label || form.article).trim() : '');
//             const newArticleSuggestions = [...articleSuggestions];
//             newArticles.forEach(article => {
//                 if (!newArticleSuggestions.includes(article)) {
//                     newArticleSuggestions.push(article);
...
//                                                         options={suggestions} // Уже преобразованы в объекты с ключом `label`
//                                                         placeholder="Введите запрос"
//                                                         defaultSelected={form.query ? [{ label: form.query.toString() }] : []} // Преобразуем в строку
//                                                         allowNew
...
//                                                         options={articleSuggestions} // Уже преобразованы в объекты с ключом `label`
//                                                         placeholder="Введите артикул"
//                                                         defaultSelected={form.article ? [{ label: form.article.toString() }] : []} // Преобразуем в строку
//                                                         allowNew
